...
**第 1 层 —— 语义文件按项目拆分。**
把 `mvt-analyze-code` 的输出从单个 `project-context.md` 改为按项目的多文件：

```
knowledge/project/_generated/project-context.md          （单项目：保持不变）
knowledge/project/_generated/{name}/project-context.md   （多项目：每项目一份）
```

这其实**简化**了 `mvt-analyze-code`：不再需要在单文件内做 section 替换同时保留其他项目，而是直接写入/替换整份项目文件。单项目仓库沿用原有扁平路径（零迁移成本）。
...
knowledge:                         # 顶层 = 所有 skill 都加载
  ...
  api:
    - id: "project-context"
      source: "knowledge/project/_generated/api/"
      files: ["project-context.md"]
...
- 多项目语义文件 `project-context` 不再放 `shared`，改放 `knowledge.<项目名>`。
...
| 6.4 | `mvt-sync-context` | 依据 change/任务所属项目，把聚合知识路由到**对应项目**的语义文件 | 今日它并回单个 `project-context.md`；拆分后必须落到正确项目文件 |
