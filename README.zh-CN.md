...
> **告别重复的上下文。功能端到端一气贯通。**
>
> 25 个专项 AI 技能，共享同一个持久化工作区 —— 覆盖从需求分析到测试交付的完整开发全流程。
...
MVTT 的 25 个技能不是 25 条互不相关的命令 —— 它们是**一个整体团队**，共用同一本项目笔记。每个技能的输出就是下一个技能的输入，上下文在叠加，不在蒸发。
...
| `/mvt-consult` | 想搞懂项目逻辑或核对某个细节 | 优先用已有知识作答，必要时查源码,不动代码 |
...
CLAUDE.md 给 Claude **指令**。MVTT 给 Claude **一个团队** —— 25 个专项技能，共享记忆、专用模板、交接协议。CLAUDE.md 是一份备忘，MVTT 是一张组织架构图。两者可以一起用，不冲突。
...
大多数团队日常只用 5–8 个。剩下的 16–20 个是给特定场景准备的（史诗级拆分、上下文同步、输出模板等），不需要一开始全学。跑 `/mvt-help` 它会告诉你下一步该用哪个。
